...
class SchwabMarketStream:
    ...
    def __init__(self, client_factory: Callable[[], SchwabClient] | None = None, max_option_symbols: int = 300) -> None:
    ...
    def wait_for_events(self, after_sequence: int, symbol: str, timeout: float = 15.0) -> tuple[int, list[dict]]:
        target = str(symbol or "").strip().upper()
        with self._condition:
            self._condition.wait_for(lambda: self._sequence > after_sequence or self._stop.is_set(), timeout=timeout)
            ...
            events = [
                ...
                and (
                    event["type"] == "status"
                    or not target
                    or event["symbol"] == target
                    or (event["type"] == "option" and event.get("underlying") == target)
                )
            ]
    ...
    async def _apply_subscriptions(self, stream: StreamClient, equities: set[str], options: set[str]) -> None:
        ...
        if new_equities:
            if equities:
                await stream.level_one_equity_add(new_equities)
                await stream.chart_equity_add(new_equities)
            else:
                await stream.level_one_equity_subs(new_equities)
                await stream.chart_equity_subs(new_equities)
            equities.update(new_equities)
        if new_options:
            if options:
